...
perlin_fill_gradients(perlin_t **per)
{
	...
	for (int i = 0; i <= arrsize; i++) {
		(*per)->gradient_array[i] = malloc(
		    (arrsize + 1) * (sizeof(GradType)));
	...
	for (int i = 0; i < 10; ++i) {
		...
		for (int j = 0; j < 10; ++j) {
			...
			if (hoveredtile->x < 5) {
				...
				mousex = ((j - (MAP_WIDTH - hoveredtile->x) +
					      1) *
					12 +
				    32);
			} else
	...
	for (int i = 0; i < 11; ++i) {
		for (int j = 0; j < 256; j++) {
			if (j == mousex && i == mousey)
				printf("\x1b[48;5;128m");
			putchar(mapgrid[i][j]);
			if (j == mousex + 6 && i == mousey)
				printf("\x1b[0m");
		}
		printf("\x1b[E");
	}
	...
	for (int i = 0; i < MAP_HEIGHT; ++i) {
		for (int j = 0; j < MAP_WIDTH; ++j) {
			...
			do {
				...
				if (cur < .08) {
					map[i][j].tile = GRASS_LONG;
					break;
				}
				if (cur < .12) {
					map[i][j].tile = ROCKS_LARGE;
					break;
				}
				if (cur < .6) {
					map[i][j].object = FOREST;
					break;
				}
				if (cur < .7) {
					map[i][j].object = FLORA;
					break;
				}
				map[i][j].tile = GRASS_SHORT;
				break;
...
float
point_dot_product(float xg, float yg, float xi, float yi)
{
...
perlin(perlin_t *per)
{
	...
	for (int i = 0; i < per->size; ++i) {
		for (int j = 0; j < per->size; ++j) {
			...
			/* within each quadrant, iterate through the rows and
			 * columns */
			for (float si = div; ystep < per->pvalsize;
			    si += div, ystep++) {
				for (float sj = div; xstep < per->pvalsize;
				    sj += div, xstep++) {
					for (int m = 0; m <= TL; ++m) {
						/* starting with the top left
						 * corner, get the associated
						 * +/- x/y modifier */
						int xmod = gmod[m].x;
						int ymod = gmod[m].y;

						gindex =
						    per->gradient_array[xmod +
							i][ymod + j];
					...
					per->cbuf[ystep + (i * per->pvalsize)]
						 [xstep + (j * per->pvalsize)] =
					    average;
					count++;
					//					printf("%d,
					//%d : ", xstep+(j*per->pvalsize),
					//ystep+(i*per->pvalsize));
				}
				xstep = 0;
			}
		}
...
place_track(maptile_t *tile, Tracks selected)
{
	if (tile->object == OBJ_NONE) {
		...
		printf("An object is in the way.\n");
}
